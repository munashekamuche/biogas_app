...
class ExportService {
  // Export application as text/PDF-like format
  Future<String> exportApplicationToText(ApplicationModel application) async {
    ...
    buffer.writeln('Service Type: ${application.serviceType.toUpperCase().replaceAll('_', ' ')}');
  ...
  Future<void> shareApplication(ApplicationModel application) async {
    try {
      ...
      throw Exception('Failed to share application: $e');
    }
  }

  // Save application to file
  Future<File> saveApplicationToFile(ApplicationModel application) async {
    try {
      final text = await exportApplicationToText(application);
      final directory = await getApplicationDocumentsDirectory();
      final file = File('${directory.path}/application_${application.id}.txt');
