...
class SyncService {
  ...
  Future<void> syncUserToLocal(UserModel user) async {
    ...
    await isar.writeTxn(() async {
      final existingUsers =
          await isar.isarUsers.filter().userIdEqualTo(user.id).findAll();
  ...
  Future<UserModel?> getUserFromLocal(String userId) async {
    ...
    try {
      final isarUsers =
          await isar.isarUsers.filter().userIdEqualTo(userId).findAll();
  ...
  Future<void> syncApplicationToLocal(ApplicationModel application) async {
    ...
    await isar.writeTxn(() async {
      final existingApps = await isar.isarApplications
          .filter()
          .applicationIdEqualTo(application.id)
          .findAll();
  ...
  Future<List<ApplicationModel>> getLocalApplications({
    ...
    var isarApps = userId != null
        ? await isar.isarApplications.filter().userIdEqualTo(userId).findAll()
  ...
  Future<void> syncPendingApplications() async {
    ...
    final pendingApps =
        await isar.isarApplications.filter().needsSyncEqualTo(true).findAll();
  ...
  Future<void> syncReportToLocal(ReportModel report) async {
    ...
    await isar.writeTxn(() async {
      final existingReports = await isar.isarReports
          .filter()
  ...
  Future<List<ReportModel>> getLocalReports({
    ...
    var isarReports = userId != null
        ? await isar.isarReports.filter().userIdEqualTo(userId).findAll()
  ...
  Future<void> syncPendingReports() async {
    ...
    final pendingReports =
        await isar.isarReports.filter().needsSyncEqualTo(true).findAll();
